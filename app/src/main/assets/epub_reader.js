...
(function () {
    function applyMobileOptimizationsAndSelection() {
        ...
        style.innerHTML = `
                    ...
                    p, div, li, td, th, span { font-size: 1em; line-height: inherit; }
                    p, ul, ol { margin-top: 0.5em; margin-bottom: 0.5em; }
                    li { margin-bottom: 0.25em; }
                    h1, h2, h3, h4, h5, h6 { line-height: 1.3; margin-top: 1em; margin-bottom: 0.5em; font-weight: bold; }
                    h1 { font-size: 2em; } h2 { font-size: 1.75em; } h3 { font-size: 1.5em; }
                    h4 { font-size: 1.25em; } h5 { font-size: 1.1em; } h6 { font-size: 1em; }
                    img, svg, video, canvas { max-width: 100%; width: 100%; height: auto; display: block; margin-left: auto; margin-right: auto; background-color: transparent; object-fit: contain; }

                    figure img { height: auto !important; }
                    div:has(> img:only-child), p:has(> img:only-child) { height: auto !important; line-height: normal !important; }

                    .tts-highlight {
                        background-color: rgba(255, 236, 179, 0.8); /* Semi-transparent Gold */
                    ...
                    /* User Highlights - NOW USING RGBA FOR BLENDING */
                    .user-highlight-yellow { background-color: rgba(255, 235, 59, 0.5); cursor: pointer; }
                    .user-highlight-green { background-color: rgba(56, 142, 60, 0.5); cursor: pointer; }
                    .user-highlight-blue { background-color: rgba(25, 118, 210, 0.5); cursor: pointer; }
                    .user-highlight-red { background-color: rgba(211, 47, 47, 0.5); cursor: pointer; }

                    /* Atomic Interaction Feedback */
                    .user-highlight-yellow:active { background-color: rgba(253, 216, 53, 0.8); }
                    .user-highlight-green:active { background-color: rgba(165, 214, 167, 0.8); }
                    .user-highlight-blue:active { background-color: rgba(144, 202, 249, 0.8); }
                    .user-highlight-red:active { background-color: rgba(239, 154, 154, 0.8); }

                    /* Dark Theme Overrides - Adjust alpha for visibility against dark bg */
                    html.dark-theme .user-highlight-yellow { background-color: rgba(251, 192, 45, 0.6); color: inherit; }
                    html.dark-theme .user-highlight-green { background-color: rgba(56, 142, 60, 0.6); color: inherit; }
                    html.dark-theme .user-highlight-blue { background-color: rgba(25, 118, 210, 0.6); color: inherit; }
                    html.dark-theme .user-highlight-red { background-color: rgba(211, 47, 47, 0.6); color: inherit; }
                `;
    ...
    function handleHighlightInteraction(e) {
        ...
        while (target && target !== document.body) {
            if (target.nodeType === Node.ELEMENT_NODE && target.classList &&
                (target.classList.contains('user-highlight-yellow') ||
                    target.classList.contains('user-highlight-green') ||
                    target.classList.contains('user-highlight-blue') ||
                    target.classList.contains('user-highlight-red'))) {
        ...
        if (highlightSpan) {
            ...
            if (window.HighlightBridge) {
                window.HighlightBridge.onHighlightClicked(
                    cfiToReport,
                    text,
                    rect.left,
                    rect.top,
    ...
    window.auditTocFragments = function() {
        ...
        console.log("FRAG_NAV_DEBUG: Sample IDs existing in DOM: " +
            Array.from(allWithId).slice(0, 5).map(el => el.id).join(", "));
    };
    ...
    window.triggerInitialScrollStateReport = function () {
        ...
        function tryReport() {
            ...
            if (currentClientHeight > 0) { setTimeout(window.reportScrollState, 50); return; }
            if (attempts < maxAttempts) { var retryDelay = baseInterval + (attempts * 50); setTimeout(tryReport, retryDelay); }
        }
    ...
    window.scrollToChapterStart = function () {
        requestAnimationFrame(function () {
            window.scrollTo(0, 0);
            setTimeout(function () { window.reportScrollState(); }, 100);
        });
    };
        ...
        while ((highlightNode = document.querySelector('.tts-highlight')) !== null) {
            ...
            if (parent) {
                try {
                    ...
                    if (highlightNode.parentNode) { // Check if it wasn't already removed by a concurrent process
    ...
    window.highlightFromCfi = function (cfi, textToHighlight, startOffset) {
        console.log(`${TTS_HIGHLIGHT_LOG_TAG}: highlightFromCfi called. CFI='${cfi}', Offset=${startOffset}, Text='${textToHighlight.substring(0, 50)}...'`);
        window.removeHighlight();
        if (!cfi || !textToHighlight) {
            const errorMsg = "JS: CFI or text missing.";
            console.log(`${TTS_HIGHLIGHT_LOG_TAG}: ${errorMsg}`);
            return errorMsg;
        }
        ...
        try {
            ...
            console.log(`${TTS_HIGHLIGHT_LOG_TAG}: Node found for CFI. Node type: ${location.node.nodeName}, Text content: '${(location.node.textContent || "").substring(0, 50)}...'`);
            ...
            console.log(`${TTS_HIGHLIGHT_LOG_TAG}: Start node for range found. Node: '${currentNode.nodeValue.substring(0, 50)}...', calculated offset: ${remainingOffset}`);
            ...
            while (remainingTextLength > 0 && endNode && sanityCheck < 50) {
                ...
                console.log(`${TTS_HIGHLIGHT_LOG_TAG}: Finding end range. Remaining text: ${remainingTextLength}, Current node: '${endNode.nodeValue.substring(0, 50)}...', available length: ${availableLength}`);
            ...
            console.log(`${TTS_HIGHLIGHT_LOG_TAG}: End node for range found. End node: '${endNode ? endNode.nodeValue.substring(0, 50) : "null"}', End offset: ${endOffset}`);


            if (remainingTextLength > 0) {
                console.log(`${TTS_HIGHLIGHT_LOG_TAG}: Text to highlight was longer than found text nodes. Highlighting to end of last found node.`);
                if (endNode) {
            ...
            try {
                console.log(`${TTS_HIGHLIGHT_LOG_TAG}: Attempting to surround content with highlight span.`);
                range.surroundContents(highlightSpan);
            } catch (e) {
                console.log(`${TTS_HIGHLIGHT_LOG_TAG}: surroundContents failed, using fallback. Error: ${e.message}`);
                const contents = range.extractContents();
                highlightSpan.appendChild(contents);
                range.insertNode(highlightSpan);
            }

            console.log(`${TTS_HIGHLIGHT_LOG_TAG}: Highlight successful. Scrolling into view.`);
    ...
    function log(message) {
        console.log(`${CFI_LOG_TAG}: ${message}`);
    }

    function resolveCfiPath(rootElement, path) {
        log(`Attempting to resolve path '${path}' from root <${rootElement.tagName}>`);
        ...
        for (let i = 0; i < steps.length; i++) {
            ...
            if (!currentNode) {
                log(`Traversal failed: currentNode became null before step ${i} (CFI index ${cfiIndex}).`);
                return null;
            }
            ...
            if (childNodeIndex >= 0 && childNodeIndex < elementChildren.length) {
                ...
                const childrenTags = elementChildren.map(node => `<${node.tagName || "TEXT"}>`).join(', ');
                log(`Step ${i}: FAILED. CFI index ${cfiIndex} (child index ${childNodeIndex}) is out of bounds for ${elementChildren.length} element children: [${childrenTags}]`);
                log(`Parent Node HTML at failure point (<${currentNode.tagName}>): ${currentNode.innerHTML.substring(0, 300)}...`); // ADD THIS LINE
    ...
    window.getNodeAndOffsetFromCfi = function (cfi) {
        ...
        try {
            ...
            log(`CFI Root is <${cfiRoot.tagName}>, Path to resolve is ${pathToResolve}`);

            let resolvedNode = resolveCfiPath(cfiRoot, pathToResolve);
            log(`Resolution attempt #1 (from CFI root) result: ${resolvedNode ? resolvedNode.tagName : 'null'}`);
            ...
            log(`Successfully resolved containing element: <${currentNode.tagName || 'TEXT_NODE'}>`);
            ...
            log(`ERROR in getNodeAndOffsetFromCfi: ${e.message}`);
            return null;
        }
    };
    ...
    window.getCfiPathForElement = function (element, charOffset) {
        ...
        try {
            ...
            if (currentNode.nodeType === Node.TEXT_NODE) {
                ...
                logStack.push(`Original offset: ${charOffset}, Cumulative offset: ${accumulatedOffset}`);
                charOffset = accumulatedOffset;
                // -----------------------------------------------------

                logStack.push(`Using its parent <${currentNode.parentNode.tagName}> for path generation.`);
                currentNode = currentNode.parentNode;
            }

            const root = document.getElementById('content-container') || document.body;

            logStack.push(`Using <${root.tagName} id='${root.id}' class='${root.className}'> as the consistent CFI root.`);
            ...
            while (currentNode && currentNode !== root && currentNode.parentNode) {
                ...
                const childrenTags = elementSiblings.map(node => `<${node.tagName || "TEXT"}>`).join(', ');
                logStack.push(`GENERATION: Parent <${parentNode.tagName}> has ${elementSiblings.length} element children: [${childrenTags}]. Current node <${currentNode.tagName}> is at index ${nodeIndex}, becoming CFI step ${cfiIndex}. Path: /${path.join('/')}`);
            ...
            logStack.push(`Final CFI generated: ${cfi}`);
            return { cfi: cfi, log: logStack };
        } catch (e) {
            logStack.push(`ERROR in getCfiPathForElement: ${e.message}`);
    ...
    window.getCurrentCfi = function () {
        ...
        try {
            ...
            debugLog.push(`Probing for CFI at coordinates: x=${viewportX}, y=${viewportY} (padding top: ${window.VIEWPORT_PADDING_TOP})`);
            ...
            debugLog.push(`Initial element found: <${topElement.tagName} id='${topElement.id}' class='${topElement.className}'>`);
            debugLog.push(`Element's text content (first 50 chars): '${(topElement.textContent || "").trim().substring(0, 50)}'`);
            ...
            if (!contentRoot.contains(topElement)) {
                ...
                debugLog.push(`Initial element was outside content root. Switched to first meaningful child: <${topElement ? topElement.tagName : 'null'}>`);
            }
            ...
            if (range && range.startContainer && range.startContainer.nodeType === Node.TEXT_NODE && range.startContainer.textContent.trim().length > 0) {
                ...
                debugLog.push(`Success with caretRangeFromPoint. Node: "${nodeForCfi.nodeValue.substring(0, 30)}...", Offset: ${offsetForCfi}`);
            } else {
                ...
                offsetForCfi = 0;
                debugLog.push(`Using fallback node for CFI generation: <${nodeForCfi.tagName || 'TEXT_NODE'}>`);
            }
            ...
            debugLog.push(`FATAL ERROR in getCurrentCfi: ${e.message}`);
        }
    ...
    window.scrollToCfi = function (cfi) {
        ...
        setTimeout(() => {
            ...
            try {
                ...
                if (location && location.node) {
                    log(`Successfully found location for CFI. Node Type: ${location.node.nodeType}, Node Name: ${location.node.nodeName}`);
                    ...
                    if (location.node.nodeType === Node.TEXT_NODE && location.offset > 0) {
                        try {
                            ...
                            if (rect.top !== 0 || rect.left !== 0) {
                                ...
                                log(`Precise scroll calculated. Rect top: ${rect.top}, Current scrollY: ${currentScrollY}, Target scrollY: ${targetScrollY}`);
                            ...
                            log(`Error during precise scroll calculation: ${rangeError.message}. Falling back.`);
                        }
                    }

                    const targetElement = (location.node.nodeType === Node.TEXT_NODE) ? location.node.parentNode : location.node;
                    log(`Using fallback scrollIntoView for element: <${targetElement.tagName}>`);
                    targetElement.scrollIntoView({ behavior: 'auto', block: 'start', inline: 'nearest' });

                    setTimeout(() => {
                        const newScrollY = window.scrollY;
                        log(`ScrollY AFTER fallback scroll: ${newScrollY}`);
                        window.reportScrollState();
                    }, 150);
    ...
    window.HighlightBridgeHelper = {
        ...
        updateHighlightStyle: function (cfi, newColorClass, colorId) {
            ...
            allSpans.forEach(span => {
                var cfis = (span.getAttribute('data-cfi') || "").split('|');
                if (cfis.includes(cfi)) {
                    if (cfis.length === 1) {
                        span.className = newColorClass;
                    } else {
        ...
        createUserHighlight: function (colorClass, colorId) {
            console.log(`${HL_LOG_TAG}: createUserHighlight. Class: ${colorClass}`);
            ...
            try {
                ...
                console.log(`${HL_LOG_TAG}: [CREATE] Raw Selection - Text: '${text}'`);
                console.log(`${HL_LOG_TAG}: [CREATE] StartContainer Type: ${safeStartNode.nodeType}, Name: ${safeStartNode.nodeName}`);
                ...
                if (startCfi !== endCfi) {
                    ...
                    console.log(`${HL_LOG_TAG}: [CREATE] Range CFI Detected. Start: ${startCfi}, End: ${endCfi}`);
                }

                console.log(`${HL_LOG_TAG}: [CREATE] Final CFI: ${finalCfi}`);
                ...
                console.log(`${HL_LOG_TAG}: Create Error: ` + e.message);
            }
        },
        ...
        removeHighlightByCfi: function (cfiToRemove, optionalCssClass) {
            console.log(`${HL_LOG_TAG}: removeHighlightByCfi called.`);
            console.log(`${HL_LOG_TAG}: -> Target CFI: '${cfiToRemove}'`);
            console.log(`${HL_LOG_TAG}: -> Optional Class: '${optionalCssClass}'`);
            ...
            console.log(`${HL_LOG_TAG}: Total highlight spans found in DOM: ${allSpans.length}`);
            ...
            allSpans.forEach(span => {
                ...
                if (cfiList.includes(cfiToRemove)) {
                    foundCount++;
                    console.log(`${HL_LOG_TAG}: Match found on span. Current CFIs: [${currentCfiAttr}]`);
                    ...
                    if (newCfiList.length === 0) {
                        ...
                        console.log(`${HL_LOG_TAG}: -> Removing span entirely (no remaining CFIs).`);
                        ...
                        console.log(`${HL_LOG_TAG}: -> Updating span (remaining CFIs: ${newCfiList.join('|')}).`);
                        span.setAttribute('data-cfi', newCfiList.join('|'));

                        if (optionalCssClass) {
                            console.log(`${HL_LOG_TAG}: -> Removing CSS class: ${optionalCssClass}`);
                            span.classList.remove(optionalCssClass);
                        } else {
                            console.log(`${HL_LOG_TAG}: -> Warning: No CSS class provided to remove. Visual style might persist if classes are mixed.`);
                        }
                        updatedCount++;
                    }
                }
            ...
            console.log(`${HL_LOG_TAG}: Removal Summary -> Matched: ${foundCount}, Removed: ${removedCount}, Updated: ${updatedCount}`);

            if (foundCount === 0) {
                console.log(`${HL_LOG_TAG}: No exact matches found. Attempting legacy fallback.`);
        ...
        restoreHighlights: function (jsonArrayString) {
            try {
                ...
                console.log(`${HL_LOG_TAG}: Error restoring: ` + e.message);
            }
        },
            ...
            try {
                ...
                if (text && text.length > 0 && startNode && startNode.nodeType === Node.TEXT_NODE) {
                    ...
                    if (substring !== text && substring.trim() !== text.trim()) {
                        console.log(`${HL_LOG_TAG}: Text mismatch at CFI. Searching nearby... Expected: '${text.substring(0, 10)}...', Found: '${substring.substring(0, 10)}...'`);

                        // Try finding the text in the whole node
                        const foundIndex = nodeVal.indexOf(text);
                        if (foundIndex !== -1) {
                            console.log(`${HL_LOG_TAG}: Found text elsewhere in node. Adjusting offset from ${startOffset} to ${foundIndex}.`);
                            ...
                            if (partialIndex !== -1) {
                                console.log(`${HL_LOG_TAG}: Found partial match. Adjusting offset.`);
...
(function() {
    ...
    window.autoScroll = {
        ...
        start: function(speed) {
            console.log(`${TAG_AUTO_SCROLL}: Starting auto-scroll. Speed: ${speed}`);
            this.active = true;
        ...
        stop: function() {
            console.log(`${TAG_AUTO_SCROLL}: Stopping auto-scroll.`);
            this.active = false;
        ...
        updateSpeed: function(newSpeed) {
            console.log(`${TAG_AUTO_SCROLL}: Speed updated to ${newSpeed}`);
            this.speed = newSpeed;
        },
        ...
        loop: function() {
            ...
            if (pixelsToScroll >= 1) {
                ...
                if (isAtBottom || isStuck || (docHeight - currentScrollPos < 50)) {
                     console.log(`${TAG_AUTO_SCROLL}: Loop Status -> ScrollY: ${Math.round(scrollY)}, InnerH: ${innerH}, Pos: ${Math.round(currentScrollPos)}, DocH: ${docHeight}`);
                     console.log(`${TAG_AUTO_SCROLL}: Check -> AtBottom: ${isAtBottom}, Stuck: ${isStuck}`);
                }

                if (isAtBottom || isStuck) {
                    console.log(`${TAG_AUTO_SCROLL}: End of chapter detected. Calling Bridge.`);
                    this.stop();
                    const container = document.getElementById('content-container') || document.body;
                    ...
                    if (window.AutoScrollBridge && window.AutoScrollBridge.onChapterEnd) {
                        window.AutoScrollBridge.onChapterEnd();
                         ...
                         console.log(`${TAG_AUTO_SCROLL}: AutoScrollBridge not found!`);
                    }
                    return;
                }
            }
            ...
            const container = document.getElementById('content-container') || document.body;
            if (container) {
                container.style.transform = `translateY(-${subPixelRemainder}px)`;
            }
